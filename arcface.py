...
# Load labels
json_open = open('train_meta.json', 'r')
...
for i in df_train.index:
    num = len(glob.glob(os.path.join("train", str(i).zfill(3), "*")))
...
class buzaiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        file = os.path.join("train", str(row.imdir).zfill(3), "{}.jpg".format(row.ids))
...
class buzaiDataset2(Dataset):
    ...
    def __getitem__(self, index):
        row = self.df.iloc[index]
        tiff_file = os.path.join("train", str(row.imdir).zfill(3), "{}.jpg".format(row.ids))
